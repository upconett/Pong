...
int PlayerVsPlayer
(
	RenderWindow &window, Clock &FPSClock, bool is_paused, 
	RectangleShape &player1, RectangleShape &player2, CircleShape &ball, RectangleShape &boom,
	RectangleShape &outline, RectangleShape &separator, Text &player1Score, Text& player2Score, 
	Sound &pauseSound, Sound &pipSound, Sound &punchSound, RectangleShape &cursor, float paddleSpeed, Vector2f &paddleSize,
	float ballSpeed, float ballAngle, int ballRadius, int score1, int score2, bool boomAnimation, int animationStart, Clock &animationClock,
	bool ballExist, Text &pauseText1, Text &pauseText2, float fade
)
{
...
)
{
	...
	bool ballUnderControl = true;

	Clock botConciousness;
	float botLastActive = 0;
	...
	while (window.isOpen())
	{
		...
		if (!is_paused)
		{
			...
			if (Keyboard::isKeyPressed(Keyboard::Up) && player1.getPosition().y >= 4 && !is_paused)
				player1.move(0, -paddleSpeed * deltaTime);
			else if (Keyboard::isKeyPressed(Keyboard::Down) && player1.getPosition().y <= 252 - paddleSize.y && !is_paused)
				player1.move(0, paddleSpeed * deltaTime);


			//Bot Inteligence

			if (ball.getPosition().y < player2.getPosition().y + paddleSize.y / 4
				&& player2.getPosition().y >= 4 && !is_paused
				&& botLastActive + 500 <= botConciousness.getElapsedTime().asMilliseconds())
			{
				ballUnderControl = false;
				botLastActive = botConciousness.getElapsedTime().asMilliseconds();
			}

			else if (ball.getPosition().y > player2.getPosition().y + paddleSize.y * 0.75
				&& player2.getPosition().y <= 252 - paddleSize.y && !is_paused
				&& botLastActive + 500 <= botConciousness.getElapsedTime().asMilliseconds())
			{
				ballUnderControl = false;
				botLastActive = botConciousness.getElapsedTime().asMilliseconds();
			...
			{
				if (ball.getPosition().y >= player2.getPosition().y + paddleSize.y / 4
					&& ball.getPosition().y <= player2.getPosition().y + paddleSize.y * 0.75)
					ballUnderControl = true;
				else if (ball.getPosition().y < player2.getPosition().y + paddleSize.y / 2.5
					&& player2.getPosition().y >= 4 && !is_paused)
				{
					player2.move(0, -paddleSpeed * deltaTime);
				}

				else if (ball.getPosition().y > player2.getPosition().y + paddleSize.y * 0.4
					&& player2.getPosition().y <= 252 - paddleSize.y && !is_paused)
				{
					player2.move(0, paddleSpeed * deltaTime);
				}
...
int main()
{
	...
	winner.setFillColor(Color(255, 255, 255, 180));
	winner.setOrigin(22, 0);
	winner.setPosition(128, 128);
			...
			case Event::KeyPressed:
				...
				case Keyboard::Up:
					...
					case GAMEMODE:
						...
						{
							gameMode = PvsP;
							cursor.setPosition(133, 188);
						}
						else
						{
							gameMode = PvsC;
							cursor.setPosition(133, 133);
						}
						break;
					}
					break;
	...
	{
	case PvsP:
		result = PlayerVsPlayer
		(
			window, FPSClock, is_paused,
			player1, player2, ball, boom, outline, separator, player1Score, player2Score,
			pauseSound, pipSound, punchSound, cursor, paddleSpeed, paddleSize, ballSpeed,
			ballAngle, ballRadius, score1, score2, boomAnimation, animationStart, animationClock,
			ballExist, pauseText1, pauseText2, fade
		);
		break;
	case PvsC:
		result = PlayerVsComputer
		(
			window, FPSClock, is_paused,
			player1, player2, ball, boom, outline, separator, player1Score, player2Score,
			pauseSound, pipSound, punchSound, cursor, paddleSpeed, paddleSize, ballSpeed,
	...
	{
	case 0:
		winner.setString("1 player win");
		break;
	case 1:
		winner.setString("2 player win");
		break;
	}
